...
import { api, RentalEquipment, RentalEquipmentInput, RentalBillingUnit } from '@/lib/api';
...
import { RentalEquipmentForm } from '@/components/rental-equipment-form';
...
export default function AlugueisPage() {
  ...
  const [editingEquipment, setEditingEquipment] = useState<RentalEquipment | undefined>();
  ...
  const loadEquipment = useCallback(
    async (searchTerm?: string) => {
      ...
      setLoading(true);
      try {
        const data = await api.listRentalEquipment(token, searchTerm);
        setEquipment(data);
      } finally {
        setLoading(false);
      }
  ...
  function openEditForm(item: RentalEquipment) {
    setEditingEquipment(item);
    setIsFormOpen(true);
  }

  async function handleFormSubmit(data: RentalEquipmentInput) {
    if (!token) return;

    if (editingEquipment) {
      await api.updateRentalEquipment(token, editingEquipment.id, data);
    } else {
      await api.createRentalEquipment(token, data);
    }
  ...
  async function handleDelete(item: RentalEquipment) {
    if (!token) return;
    if (!confirm(`Excluir o equipamento "${item.name}"?`)) return;

    await api.deleteRentalEquipment(token, item.id);
    loadEquipment(search);
  }
  ...
  async function handleBulkAdjust(e: React.FormEvent) {
    ...
    setBulkSaving(true);
    try {
      const result = await api.bulkAdjustRentalEquipmentPrices(token, { percentage, category: bulkCategory || undefined });
      alert(`${result.adjusted} equipamento(s) reajustado(s).`);
      setIsBulkOpen(false);
      setBulkPercentage('');
      setBulkCategory('');
      loadEquipment(search);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Aluguéis e Locações</h1>
          <p className="text-muted-foreground">Base de preços de equipamentos alugados.</p>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Período Mín.</TableHead>
            ...
            {loading ? (
              ...
              equipment.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{formatCurrency(item.unitPrice)}{billingUnitLabels[item.billingUnit]}</TableCell>
                  <TableCell>{formatCurrency(item.mobilizationCost)}</TableCell>
                  <TableCell>{item.minimumPeriod}</TableCell>
